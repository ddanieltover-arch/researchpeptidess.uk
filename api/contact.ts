...
export default async function handler(
  ...
  try {
    const { handleContactCreate } = await import('../src/server/contact-http');
    await handleContactCreate(req as never, res as never);
  } catch (error) {
    send(res, 500, { error: 'The enquiry could not be stored.', detail: loadError(error) });
  }
}
