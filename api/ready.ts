...
export default async function handler(
  ...
  const variables: Record<string, 'PRESENT' | 'MISSING' | 'OPTIONAL'> = {
    ...
    ADMIN_EMAIL: present('ADMIN_EMAIL') ? 'PRESENT' : 'OPTIONAL',
    ADMIN_PASSWORD: present('ADMIN_PASSWORD') ? 'PRESENT' : 'MISSING',
    SESSION_SECRET: present('SESSION_SECRET') ? 'PRESENT' : 'OPTIONAL',
    JWT_SIGNING_KEY: present('JWT_SIGNING_KEY') ? 'PRESENT' : 'OPTIONAL',
    ...
    RESEND_API_KEY: present('RESEND_API_KEY') ? 'PRESENT' : 'MISSING',
